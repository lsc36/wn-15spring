...
public class Camera2VideoFragment extends Fragment implements View.OnClickListener {
    ...
    private TextureView.SurfaceTextureListener mSurfaceTextureListener
        ...
        public void onSurfaceTextureUpdated(SurfaceTexture surfaceTexture) {
            /*Log.d(TAG, "frame " + mCounter);
            mTextureView.getBitmap(mBitmap);
            if (mCounter < 30) {
                try {
                    String filename = new File(getActivity().getExternalFilesDir(null),
                            String.format("%02d.jpg", mCounter)).getAbsolutePath();
                    BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(filename));
                    mBitmap.compress(Bitmap.CompressFormat.JPEG, 90, bos);
                    bos.close();
                    Log.d(TAG, "frame saved to " + filename);
                } catch (Exception e) {
    ...
    private static Size chooseVideoSize(Size[] choices) {
        for (Size size : choices) {
            if (size.getWidth() == size.getHeight() * 4 / 3 && size.getWidth() <= 1024) {
                return size;
            }
        }
    ...
    private void configureExposure() {
        try {
            ...
            mPreviewBuilder.set(CaptureRequest.SENSOR_EXPOSURE_TIME, (long) 200000);
            mPreviewBuilder.set(CaptureRequest.SENSOR_SENSITIVITY, 100);
            mPreviewBuilder.set(CaptureRequest.LENS_APERTURE, (float) 2.8);
        } catch (Exception e) {
            Log.e(TAG, e.getMessage());
        }
    }
    ...
    private void stopRecordingVideo() {
        ...
        Runnable sender = new Runnable() {
            ...
            public Runnable init(ProgressDialog pd) {
                progressDialog = pd;
                return this;
            }
            ...
            public void run() {
                try {
                    ...
                    MultipartUtility mu = new MultipartUtility(
                            "http://ctf.tw:25252", "UTF-8");
                    mu.addFilePart("video", getVideoFile(getActivity()));
                    List<String> response = mu.finish();
                    Log.d(TAG, "end sending request");
                    progressDialog.dismiss();
                    String message;
                    try {
                        message = response.get(0);
                    } catch (IndexOutOfBoundsException e) {
                        message = "Nothing decoded";
                    } catch (Exception e) {
                    ...
                    activity.runOnUiThread(new Runnable() {
                        ...
                        public Runnable init(Activity act, String msg) {
                            activity = act;
                            message = msg;
                            return this;
                        }
